...
public interface ExecutorService extends Executor {
    /**
     * 停止线程池，状态设置为 SHUTDOWN，并且不再接受新的任务，已经提交的任务会继续执行。
    ...
    /**
     * 如果关闭后所有任务都已完成，则返回 true。请注意，除非首先调用 shutdown() 或 shutdownNow()，否则 isTerminated() 永远不会是true。
    ...
    /**
     * 阻塞直到关闭请求后所有任务都已完成执行，或者发生超时，或者当前线程被中断，以先发生的为准。
     ...
     * you can use constructions of the form {@code result = exec.submit(aCallable).get();}
     *
     * <p>
     * Note: The {@link Executors} class includes a set of methods that can convert some other common closure-like objects,
     ...
     * Note that a <em>completed</em> task could have terminated either normally or by throwing an exception.
     ...
     * of one that has completed successfully (i.e., without throwing an exception), if any do.
