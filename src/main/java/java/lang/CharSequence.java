...
public interface CharSequence {
     ...
     *                                   <tt>length()</tt>
     */
    // 返回指定索引处的char值，索引的范围从0到length()-1。序列的第一个char值在索引0处，第二个在索引1处，依此类推，与数组索引一样。
     ...
     */
    // 返回此序列的子序列，子序列以start处的char值开始，并且以end-1处的char值结束。返回的子序列长度为end-start，因此，如果start == end，则返回空序列
     ...
     */
    // 返回包含此序列中字符的字符串，此字符串的顺序与此序列相同，字符串的长度将是此序列的长度。返回完全由此字符序列组成的字符串
    ...
    // 即：接口中也可以有实现方法，但需用default关键字来修饰，实现此接口的类可以直接使用该实现方法，也可以重写这个方法
