...
/**
 * String类用了final修饰符，表示它不可以被继承。同时还实现了三个接口：
 * 实现Serializable接口表示String类可被序列化；
 * 实现Comparable<T>接口主要是提供一个compareTo()方法用于比较String字符串；
 * 实现CharSequence接口，这个接口代表的是char值的一个可读序列。
 * CharBuffer、Segment、String、StringBuffer、StringBuilder也都实现了CharSequence接口。
 */
public final class String implements java.io.Serializable, Comparable<String>, CharSequence {
     ...
     */
    // 根据char数组对象构造String实例：将char数组中的元素copy到新实例的value中，而不是直接赋值。
    // Arrays.copyOf()方法是一个native方法，实际调用C语言的复制函数，效率较高。
     ...
     */
    // 类似String(char value[])构造方法，只是从偏移量offset处开始复制，复制的总长度为count，当参数错误时会抛异常
     ...
     * @since 1.5
     */
    // 类似String(char value[], int offset, int count)构造方法，只是参数为int数组，需要注意的是保证数组元素为合法的Unicode代码点(Unicode code points)
     ...
     */
    // 以StringBuffer作为入参的构造方法：注意StringBuffer是线程安全的，在使用它的对象来实例化String时，也添加了synchronized关键字以保证其线程安全。
     ...
     * @since 1.5
     */
    // StringBuilder作为入参的构造方法：注意String(StringBuffer buffer)构造方法的区别
     ...
     */
    // 包私有构造函数，共享值数组以提高速度。与String(char value[])构造方法的区别如下：
    // 1、增加一个实质上无用的boolean参数，因为没有这个参数，方法不能重载；
    // 2、直接使用“＝”赋值，速度更快且共享内存以节约空间。
    // 注意：String类的concat(String str)方法中就是用这个构造方法返回连接完成后的新String实例。
     ...
     * The length is equal to the public boolean equals(Object anObject) {number of <a href="Character.html#unicode">Unicode
     ...
     */
    // String类重写了equals()方法，将此字符串与指定的对象比较。
    // 当且仅当该参数不为null、且与此对象表示相同字符序列的String对象时，结果才为true。
    public boolean equals(Object anObject) {
        ...
        if (anObject instanceof String) {
            ...
            int n = value.length;
            if (n == anotherString.value.length) {// 当前对象与anObject的字符数量相同才有可比性
                char v1[] = value;
     ...
     */
    // 重写java.lang.Comparable中的方法，实现按字典顺序比较两个字符串
    // 逐位比较两个字符串的字符序列，如果某一位字符不相同，则返回该位的两个字符的Unicode差值，所有位都相同则计算两个字符串长度之差，两个字符串相同则返回0
    public int compareTo(String anotherString) {
        int len1 = value.length;
        int len2 = anotherString.value.length;
        int lim = Math.min(len1, len2);// 取2个String的中较小的长度作为循环比较的次数
        char v1[] = value;
        char v2[] = anotherString.value;

        int k = 0;
        while (k < lim) {// 取2个String的中较小的长度作为循环比较的次数
            // 将两个字符串的字符序列value逐个比较，如果不等则返回该位置两个字符的Unicode差值
            char c1 = v1[k];
            char c2 = v2[k];
            // 按序比较2个char的Unicode值，如果不相等则返回二者的Unicode差值(当前对象减去要比较的对象)
            if (c1 != c2) {
                return c1 - c2;
            }
            k++;
        }
        // 如果按照较小长度循环比较完成(即：比较的char完全相同)，则返回2个String的长度差值(当前对象的长度减去要比较对象的长度)
     ...
     * @since 1.2
     */
    // 一个比较器，用来排序String对象，compareToIgnoreCase()方法中有使用
     ...
     * @since 1.2
     */
    // 该方法的实现与int compareTo(String anotherString)类似，只不过比较时忽略字符的大小写
     ...
     * @since 1.4
     */
    // String的实现：调用String的substring(int beginIndex, int endIndex)方法做截取
     ...
     */
    // 当调用该方法时，如果池中已经包含一个等于此String对象的字符串(用equals(Object)方法确定)，则返回池中的字符串。否则将此String对象添加到池中,并返回此String对象的引用
    // 所有字面值字符串和字符串赋值常量表达式都使用该方法进行操作，如：String str1 = "123";
