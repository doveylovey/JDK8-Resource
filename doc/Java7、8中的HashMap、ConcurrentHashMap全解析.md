...
===============================================
来源：https://javadoop.com/post/hashmap
...
![Java7 HashMap 结构图](pic/Java7-HashMap结构图.png)
...
![Java7 ConcurrentHashMap 结构图](pic/Java7-ConcurrentHashMap结构图.png)  
...
B、remove 操作的线程安全性：  
   1、remove 操作我们没有分析源码，所以这里说的读者感兴趣的话还是需要到源码中去求实一下的。  
   2、get 操作需要遍历链表，但是 remove 操作会"破坏"链表。  
   3、如果 remove 破坏的节点 get 操作已经过去了，那么这里不存在任何问题。  
   4、如果 remove 先破坏了一个节点，分两种情况考虑：  
...
来一张图简单示意一下吧：
![Java8 HashMap 结构图](pic/Java8-HashMap结构图.png)
...
我们先用一个示意图来描述下其结构：  
![Java8 ConcurrentHashMap 结构图](pic/Java8-ConcurrentHashMap结构图.png)  
